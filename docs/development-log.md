...
## 2026-09-20 — /ops 观测台换肤 + 调用链展开交互（顺手修掉两处主题残留）

**任务**：`/ops` 换肤 + 工具调用链展开交互（TapNow 三条视觉手法落地，取值来源＝知识库卡 26027，为真浏览器实测而非臆测）。

### 做了什么

1. **唯一强调色**：页面整体灰阶，橙色只用于「会话选中条 · 步骤耗时占比 · 主操作」；没有引入第二个品牌色。
2. **描边分层代替阴影**：`.panel / .metric` 去掉 `box-shadow`（原 `--shadow-sm` 是 `rgba(20,45,110,…)` **冷蓝**阴影，压在 `#0b0d11` 深底上发脏）→ 改 `1px solid var(--border)`；登录卡同理。
3. **调用链展开交互**：轮次卡默认展开最近一轮；步骤改为可单独折叠的 `<details>` 并带**耗时占比条**与「最慢」徽章；工具参数 >180 字符默认收起（短参数保持展开）；工具返回 >260 字符或 >6 行改为渐隐折叠 + 「展开全文」；详情顶部加「展开全部 / 收起全部」。全部用**事件委托**，`innerHTML` 重建后无需重新绑定。
4. **顺手修掉两处主题残留（原换肤遗漏）**：
   - `.topbar` / `.composer` 背景仍是 `rgba(255,255,255,.78/.86)` 半透明白 → 深色页面里两条白带（正是顾问 2026-09-20 裁定点名的「顶部及底部仍有明显浅灰区域」）→ 改 `rgba(18,21,28,…)`。
   - `--warn-bg:#12151c8E1` 是**无效 CSS 值**（9 位 hex，浏览器直接丢弃该声明）→ 修正为 `rgba(242,201,76,.15)`。

### 怎么验的（不接受"看着像对的"）

- `tools/_shot_ops_theme.py`：改前/改后各 4 张 1440×900 全页截图（CDP 视口覆盖 + **回读校验**，对不上拒绝产出），落在 `_evidence/ops-theme-20260920/{before,after}/`。
- `tools/_diff_shots.py`：逐像素对比 + 差异包围盒 → 4 张图差异 bbox **全部 x ≥ 248**（侧边栏未被触碰）；ops-default 39.21%、ops-expanded 23.97%、chat-default 16.88%（＝有意修的顶栏+输入区两块）。
- `tools/_verify_ops_ui.py`：**14/14 断言通过**（含「收起全部→展开数归零」「展开全部→全开」「长参数收起 / 短参数展开**两条分支**」「展开全文按钮可来回还原」「无 JS 运行时错误」），在**临时实例 8011 与正式端口 8010 各跑一遍**。

### 坑（下次直接照做）

- 用户常驻的 CDP Chrome（9224）会 `<ws connected>` 之后不响应、`connect_over_cdp` 180s 超时 → 截图链路**不要依赖它**，改用 `playwright launch(channel='chrome', headless=True, args=['--no-proxy-server'])` 自起实例（本机系统代理会劫持 127.0.0.1）。
- 「展开第一轮」若直接点 `summary`，在**默认已展开**的轮次上会变成**收起**（截图哈希不同但语义相反）→ 截图脚本先 `collapse-all` 再展开，并把「展开元素数」写进清单供核对。
- `main.py` 是内嵌 HTML 的单文件，两页（`_USER_PAGE` / `_OPS_PAGE`）有大量**逐字相同**的 CSS → `patch` 的 `old_string` 会命中两处，须用「`</style>` + `</head>` + `<body>` + 该页特有行」组合定位，或按 `_OPS_PAGE = r"""` 起点做偏移插入。
- 改 `main.py` 后**必须重启 8010**（uvicorn 未开 `--reload`）；验证期可先起临时实例 8011，确认后再重启正式端口，避免打断演示环境。
