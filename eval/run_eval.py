...
# ── 评估用的请求上下文 ────────────────────────────────
# 图调用需要显式传入身份上下文（不再有模块级全局变量可设）。
# 评估要真的打到下游 p2/rag，所以必须带一个**下游能验签通过**的 token ——
# 用本服务的 AUTH_SECRET 自签一个专用测试身份，而不是复用某个真人账号。
EVAL_OWNER_ID = "eval-owner"
...
def _make_eval_ctx():
    ...
    if not secret:
        raise SystemExit(
            "AUTH_SECRET 未配置：评估会真的打到下游 p2/rag 并自签测试身份，"
            "必须与三个服务使用同一个密钥（见 .env.example）"
        )
    token = pyjwt.encode(
        {
            "sub": "eval-runner",
    ...
    return make_context("eval-" + uuid.uuid4().hex[:8], token, EVAL_OWNER_ID)


EVAL_CTX = _make_eval_ctx()
...
for case in test_cases:
    ...
    try:
        answer, actual_tools = run_supervisor_with_trace(msg, EVAL_CTX)
...
# ── 结果快照（机器可读，README 与所有对外表述只引用这里，禁止手写数字）────
