...
# 三个仓库默认克隆在同一个父目录下（见 README「最短启动」）
TICKET_ROOT = os.path.join(os.path.dirname(ORCH_ROOT), "customer-service-agent")
...
def test_case(msg: str, label: str) -> None:
    ...
    # 图调用需显式传入身份上下文；本脚本要真打下游服务，故自签一个能验签通过的测试身份
    import jwt as _jwt
    import uuid as _uuid

    _secret = os.getenv("AUTH_SECRET")
    if not _secret:
        raise SystemExit("AUTH_SECRET 未配置：本脚本要真打下游服务，需要与三服务同一个密钥")
    _tok = _jwt.encode(
        {"sub": "e2e-runner", "role": "admin", "owner_id": "e2e-owner",
         "exp": int(time.time()) + 3600},
        _secret, algorithm="HS256",
    )
    ctx = make_context("e2e-" + _uuid.uuid4().hex[:8], _tok, "e2e-owner")

    t0 = time.time()
    answer, tools_used = run_supervisor_with_trace(msg, ctx)
